...
CellGrid::CellGrid(sf::Vector2i dimensions) {
	...
	chunk_rect.setOutlineColor(sf::Color(255, 0, 255, 40));
...
void CellGrid::cell_logic(int x, int y) {
	...
	//vertical travel
	short vt = elements[i]->gravity_direction;
	short desired_spread = elements[i]->desired_spread;

	if (bounded_y && elements[i]->travel_down) {

		if (grid[x][y + vt].index == -1 || elements[grid[x][y + vt].index]->density < elements[i]->density) { //DOWN
			//move to empty place
	...
	if (bounded_y && bounded_x && elements[i]->travel_down_diagonal) { //DIAGONAL
		if (grid[x][y].prefer_left) {
			if (grid[x + 1][y + vt].index == -1 || elements[grid[x + 1][y + vt].index]->density < elements[i]->density) {
				grid[x][y].prefer_left = false;
				swap_cells(sf::Vector2i(x, y), sf::Vector2i(x + 1, y + vt));
				return;
			}
			else if (grid[x - 1][y + vt].index == -1 || elements[grid[x - 1][y + vt].index]->density < elements[i]->density) {
	...
	for (int x = 0; x < chunk.rect.getSize().x; x++) {
		for (int y = 0; y < chunk.rect.getSize().y; y++) {
			...
			if (grid[_x][_y].index != -1 && !grid[_x][_y].updated) {
				...
				world_graphic.setPixel(_x, _y, elements[caught_cell.index]->colour);
			}
		}
	}
	if (chunk.awake) {
		surface.draw(chunk.rect);
	}
